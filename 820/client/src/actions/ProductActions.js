...
export const productCreate = (values, Calories) => {
    return dispatch => {
    axios.post(process.env.REACT_APP_API_URL+"/products", values, Calories).then(
        res=> { 
...
export const productUpdate = (id, values, Calories) => {
    return dispatch =>{
        axios.put(process.env.REACT_APP_API_URL+"/products/" + id, values, Calories).then(
            res => {
