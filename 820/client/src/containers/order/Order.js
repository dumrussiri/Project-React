        ...
        return this.props.orders && this.props.orders.map(order => {
            ...
            return (
                <div key={order.id}className="col-md-3">
                    ...
                    <h5>วันที่ {date.toDateString()} {date.toLocaleTimeString()}</h5>

                    <ul>
                        {order.orders && order.orders.map(record => {
                            return (<li key={record.product.id}>
                                {record.product.productName} x {record.quantity} = {record.product.unitPrice * record.quantity} THB : {record.product.Kilocalories * record.quantity} Kcals
                            </li>)
                        })}
                    </ul>
                    <p className="title">ยอดรวม {order.totalPrice} THB : จำนวน Kcals ที่ได้จากOrder {order.totalCalories} Kcals</p>
                </div>
            )
        })
    }
